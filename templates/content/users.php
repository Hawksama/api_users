...
<article class="users-listing-wrapper">
	...
	<header class="entry-header has-text-align-center header-footer-group">
		...
		<div class="entry-header-inner section-inner medium">
			<h1 class="entry-title"><?= __('Users list from API', 'carabus'); ?></h1>
		...
		<div class="entry-content">
			...
			<div class="table-wrapper alignwide">
				<table id=data-table-users class="alignwide">
					...
					$request = new WP_REST_Request( 'GET', '/carabus/plugin/users' );
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		targetedTableID.find('tbody').on('click', '> tr', function () {
			...
			$.ajax({
				type: 'GET',
				url: carabusAjax.restURL + 'carabus/plugin/user?id=' + data['id'],
				beforeSend: function (xhr) {
					xhr.setRequestHeader ('X-WP-Nounce', carabusAjax.restNounce);
				},
